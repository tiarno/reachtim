...
This article shows how to create a set of charts for monitoring one or more machines. It uses Python/psutil, MongoDb, and jquery.

[TOC]

I need to watch a couple of FreeBSD machines to make sure they're healthy and not running into memory or disk space issues. 
I don't want to go to each machine and run `top` or `ds` to find out what's going on, I want a web page with up-to-date charts that I can glance at.
...
In the first part, I use the `psutil` package inside a `cron` job to write system information to a capped collection in MongoDB every 5 minutes. I have a collection for each machine, and it has `maxsize` set to 576 documents.
48 hours is 2,880 minutes and we sample every 5 minutes so we need space for 576 records. I estimated a generous size for the documents because this is just a start and I may want to gather more data later on.
...
The NOSQL database [mongoDB](http://www.mongodb.org/) is an open-source document database. If you have data that naturally fits a JSON-style structure, mongoDB is a great fit for data storage.

The `bottle` web [framework](http://bottlepy.org/docs/dev/index.html) is a tiny framework written in Python with no other dependencies. You can install it with `pip`. You can use the included development server to get things going and put it under a different backend server later. 


The [jqplot](http://www.jqplot.com/) `jquery` plugin makes producing the charts easy, plus they look good and they are all uniform so it easy to compare charts. For example, it is easy to see when a process spins up because you can see the spike for cpu and memory at the same moment in time which is the same x-axis location for both charts. There is a lot more you can do with this plugin, but I just want to the charts up and running and it is very easy to do that with `jqplot`.

### Get the Data {: .article-title}

We want to create data structure that follows this pattern. The main thing is to create time series so getting the timestamp to along with the data is the point here. You can add or remove any data that psutil supports in this step. As my first step I wasn't sure of what I needed and would use and so this is what I started out with. I figured that as time went by and I saw the actual needs of the machine this would change, and could be easily changed.
...
The third member is just an arbiter and doesn't keep the data.
...
Now call psutil for every piece of data you want and add it the the `doc` dictionary. Finally, add the that dictionary as a document into the corresponding MongoDb collection.
...
Create a capped MongoDB collection for each server to hold the data. In the mongo shell, use the `createCollection` command:

```javascript
db.createCollection("monitor01", 
    { capped : true, size : 5242880, max : 14400} )
db.createCollection("monitor02", 
    { capped : true, size : 5242880, max : 14400} )
```
...
Each mongoDB collection contains 48 hours of system performance data about the corresponding server. By using a capped collection, we are guaranteed that the data will be preserved in insertion order, and old documents are automatically removed as time goes by so we always have the latest 48 hours of data.

### Display the Data with jqplot {: .article-title}

The complete code is in the GitHub project, but here is a snippet of how `jqplot` is set up to show the data. The machine `example0` runs the web server that will return the json load data. The complete code is in the file `psmonitor.js` and it all follows the same pattern:
...
The HTML is very simple. Read in the stylesheet, write the `div` to hold each chart, then load the javascript.
...
### Setting up the bottle Server {: .article-title}

Create a `bottle` application to tie everything together. Connect to MongoDb and when it receives a request for server data, return the formatted data for the appropriate server in the response.  
```
